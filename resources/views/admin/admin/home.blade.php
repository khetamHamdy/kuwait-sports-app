...
@section('title') {{ucwords(__('admins'))}}
@endsection
@section('css')
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" value="{{request('email')?request('email'):''}}" name="email" placeholder="{{__('email')}}">
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label class="control-label">{{__('Phone')}}</label>
                                                <input value="{{request('mobile')?request('mobile'):''}}" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')"  type="text" class="form-control" name="mobile" placeholder="{{__('mobile')}}">
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <button type="submit" class="btn sbold btn-default btnSearch">{{__('Search')}}
                                                <i class="fa fa-search"></i>
                                            </button>

                                            <a href="{{url('/admin/admins')}}" type="submit" class="btn sbold btn-default btnRest">{{__('Reset')}}
                                ...
                                @forelse($items as $one)
                                    <tr class="odd gradeX" id="tr-{{$one->id}}">
                                        ...
                                        <td class="v-align-middle wd-15p optionAddHours">
                                            ...
                                            <a href="{{url(getLocal().'/admin/admins/'.$one->id.'/edit_password')}}" class="btn btn-sm btn-clean btn-icon" title="{{__('Change_Password')}}">
                                                <i class="la la-key"></i>
                                            </a>
